...
@Component
public class DetailRegistrationHandler implements DetailMetadataHandler {
    ...
    public Object parseMetadata(Document document) {
        ...
        DocumentDetailResponseDto.RegistrationDto dto = objectMapper.convertValue(document.getMetadata(), DocumentDetailResponseDto.RegistrationDto.class);
        if (document.getImages() != null) {
            for (Image img : document.getImages()) {
                if (img.getImageType() == ImageType.REGISTRATION_FRONT) {
